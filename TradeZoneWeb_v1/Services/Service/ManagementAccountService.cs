...
namespace TradeZoneWeb_v1.Services.Service
{
    public class ManagementAccountService : IManagementAccountService
    {
        ...
        public ManagementAccountService(IHttpClientFactory httpClientFactory, IConfiguration config)
        {
        ...
        public async Task<object> PutAccount(Guid id, PutAccountRequest request, string jwt)
        {
            var client = _httpClientFactory.CreateClient();
            client.BaseAddress = new Uri(_config["BaseUrl"]);
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", jwt);
